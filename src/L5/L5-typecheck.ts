...
//---------added by me----------------------------
export const isTypePredApp = (e: Exp, tenv: TEnv): Result<PredicateTExp> => 
    bind(typeofExp(e, tenv), (te: TExp) => isPredicateTExp(te) ? makeOk(te) : makeFailure("type predicate not found"))


export const typeofIf = (ifExp: IfExp, tenv: TEnv): Result<TExp> =>
    either(
        bind (isTypePredApp(ifExp.test, tenv), (te: PredicateTExp) => makeOk(te.texp)),
        makeOk,
...
export const typeofApp = (app: AppExp, tenv: TEnv): Result<TExp> =>
    bind(typeofExp(app.rator, tenv), (ratorTE: TExp) => {
        if (! isProcTExp(ratorTE) && !isPredicateTExp(ratorTE)) {
