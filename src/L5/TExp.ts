...
import { all, append, chain, concat, equals, map, sort, uniq, zip, filter, includes, any } from "ramda";
...
export type CompoundTExp = ProcTExp | PredicateTExp | TupleTExp | UnionTExp |InterTExp;
export const isCompoundTExp = (x: any): x is CompoundTExp => isProcTExp(x) || isTupleTExp(x) || isPredicateTExp(x) || isUnionTExp(x) || isInterTExp(x);

export type NonTupleTExp = AtomicTExp | ProcTExp | TVar | UnionTExp | InterTExp | PredicateTExp;
export const isNonTupleTExp = (x: any): x is NonTupleTExp =>
    isAtomicTExp(x) || isProcTExp(x) || isTVar(x) || isUnionTExp(x) || isInterTExp(x) || isPredicateTExp(x);
...
export type InterTExp = { tag: "InterTExp"; components: TExp[]};        // added by me
...
export type PredProcTExp = PredicateTExp | ProcTExp;
export type PredicateTExp = { tag: "PredicateTExp"; texp: TExp};        // added by me
export const makePredicateTExp = (texp: TExp): PredicateTExp =>
    ({tag: "PredicateTExp", texp:texp});
...
const flattenInter = (tes: TExp[]): TExp[] => {
    if (tes.length > 0) {
        if(tes.length === 2){
            if(isUnionTExp(tes[0]) || isUnionTExp(tes[1])){
                return tes ;
            }
        }
        if(isInterTExp(tes[0])) {
            return  [...tes[0].components, ...flattenInter(tes.slice(1))] 
}
        return [tes[0], ...flattenInter(tes.slice(1))] 
    }
    return []};
...
const normalizeInter = (inter: InterTExp): TExp =>          // added by me
    isEmpty(inter.components) ? makeNeverTExp() : 
    includes(makeNeverTExp(), inter.components) ? makeNeverTExp() : 
    includes(makeAnyTExp(), inter.components) ? (isAnyTExp(inter.components[0])? inter.components[1] : makeAnyTExp()) :
...
// [number, union( union(number, string),number ] => [number, string]
const flattenUnion = (tes: TExp[]): TExp[] => 
    (tes.length > 0) ? 
        isUnionTExp(tes[0]) ? [...tes[0].components, ...flattenUnion(tes.slice(1))] :
        //isInterTExp(tes[0]) ? [...replacInter(tes[0]), ...flattenUnion(tes.slice(1))] :   //may be an error============================
        //isInterTExp(tes[0]) ? [...flattenInter(tes)] :   //may be an error============================
        [tes[0], ...flattenUnion(tes.slice(1))] :
    [];


const replacInter = (tes: InterTExp): TExp[] => //need to change  // added by me
    {
        if(tes.components.length === 0)
            return  [makeNeverTExp()];
        if(tes.components.length === 1)
            return  [tes.components[0]];
        const te1 = tes.components[0];
        const te2 = tes.components[1];
        return [makeDiffTExp(makeUnionTExp([te1,te2]),makeUnionTExp([makeDiffTExp(te1,te2), makeDiffTExp(te2,te1)])) ];
    };

// Implementing makeDiffTExp
export const makeDiffTExp = (te1: TExp, te2: TExp): TExp => {      // added by me - need to check
    // not support 'any' data type
    if(isAtomicTExp(te1)){
        return isSubType(te1, te2) ? makeNeverTExp() : te1;
    };
    if (te2.tag === 'NeverTExp') {
        return te1;
    };
    // if (isSimpleTExp(te1) && isSimpleTExp(te2)) {
    //     return te1.tag === te2.tag ? makeNeverTExp() : te1;
    // }
    if (te1.tag === 'UnionTExp') {
        return makeUnionTExp(te1.components!.filter(c1 => !isSubType(c1, te2)))
    };
    if (te1.tag === 'InterTExp') {
        return makeInterTExp(te1.components!.map(c1 => makeDiffTExp(c1, te2)));
    };
...
const removeDuplicatesAndAny = (tes: TExp[]): TExp[] =>     // added by me
    isEmpty(tes) ? tes :
...
export const dnf = (te: TExp): TExp => 
    isInterTExp(te)  ?
         makeDnf(filter(isUnionTExp, te.components), 
...
const parsePredicateTExp =(texps: Sexp[]): Result<PredProcTExp> =>
    (texps.length === 4) && (texps.indexOf('->') === 1) && (texps.indexOf('is?') === 2) ? 
        bind(parseTExp(texps[0]), 
            (t1: TExp) => isAnyTExp(t1) ? 
                bind(parseTExp(texps[3]), 
                    (returnTE: TExp) => makeOk(makePredicateTExp(returnTE))) : 
                makeFailure("First expression isn't any!")) :
        makeFailure("cannot parse type predicate");
...
*/
const parseProcTExp = (texps: Sexp[]): Result<PredProcTExp> => {
    const pos = texps.indexOf('->');
    return (texps.indexOf('is?') !== -1) ? parsePredicateTExp(texps) :
    ...
    const up = (x?: TExp): Result<string | string[]> =>
        ...
        isProcTExp(x) ? bind(unparseTuple(x.paramTEs), (paramTEs: string[]) =>
                            mapv(unparseTExp(x.returnTE), (returnTE: string) =>
                                [...paramTEs, '->', returnTE])) :
        isPredicateTExp(x)? bind(unparseTExp(x.texp), (returnTE: string) => makeOk(`is? ${returnTE}`)) :
