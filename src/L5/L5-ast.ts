...
import { isTVar, makeFreshTVar, parseTExp, unparseTExp, TExp, makePredicateTExp } from './TExp';
...
// (lambda (<vardecl>*) [: is? <TExp>]? <cexp>+ )
// (lambda (<vardecl>*) [: <TExp>]? <CExp>+)
const parseProcExp = (vars: Sexp, rest: Sexp[]): Result<ProcExp> => {
    if (isArray(vars)) {
        ...
        const returnTE = rest[0] === ":" ? rest[1] === "is?" ? bind(parseTExp(rest[2]), (returnTE: TExp) => makeOk(makePredicateTExp(returnTE)) ) : parseTExp(rest[1]) : makeOk(makeFreshTVar());
